...
/** This repository's Pulumi project. Its stacks are its own; the organization is not. */
const PROJECT = 'demo';

export const deploy = async (stack: string, commit: string): Promise<void> => {
  const organization = requireEnvironment('PULUMI_ORGANIZATION');

  const remote = await RemoteWorkspace.createOrSelectStack(
    {
      stackName: `${organization}/${PROJECT}/${stack}`,
  ...
  if (summary.result !== 'succeeded') {
    throw new Error(`Deploying ${stack} ended as ${summary.result}`);
  }
};
